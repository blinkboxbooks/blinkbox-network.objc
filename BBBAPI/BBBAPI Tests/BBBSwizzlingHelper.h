 ...
 *  with the `block`. Must be followed by `BBB_DESWIZZLE_INSTANCE_METHOD` which 
 ...
 */
#define BBB_SWIZZLE_INSTANCE_METHOD_WITH_BLOCK(class, selector, block) Method originalMethod = class_getInstanceMethod(class,selector); \
IMP newImplementation = imp_implementationWithBlock(block); \
IMP oldImp = class_replaceMethod(class, selector, newImplementation, method_getTypeEncoding(originalMethod))

/**
 *  This macros deswizzles the method replaced with `BBB_SWIZZLE_INSTANCE_METHOD_WITH_BLOCK`
 */
#define BBB_DESWIZZLE_INSTANCE_METHOD(class, selector) class_replaceMethod(class, selector, oldImp, method_getTypeEncoding(originalMethod))